...
class Chat(BaseModel):
    ...
    def __str__(self) -> str:
        return (
            self.name or f"{'Группа' if self.is_group else 'Приватная'} Чат #{self.id}"
        )
...
class ChatParticipant(models.Model):
    ...
    chat = models.ForeignKey(
        Chat, on_delete=models.CASCADE, related_name="participants", verbose_name="Чат"
    )
...
class MessageAttachment(models.Model):
    ...
    file = models.FileField(
        "Файл (включая изображения)", upload_to="chat_files/", blank=True, null=True
    )
